...
* **模式 A，在 Termux 宿主机跑**：取宿主机参考值 → 源码拷进容器 → 用**容器自带 gcc** 编译 → 跑断言
* **模式 B，在容器内部跑**：直接对当前环境跑同样的断言

脚本严格区分两类东西：

| | 内容 | 处理方式 |
|---|---|---|
| **程序不变量** | 退出码 0/1/2、不带 `-v` 时 stderr 为空、`-v` 的 stderr 只有 `ipinfo: ` 诊断行、JSON 结构完整、无 `if<N>` 占位符、输出非空、**要了邻居表却没有数据时必须说明原因（不能静默）** | **断言**，失败即真 bug |
| **环境差异** | proot 下邻居表条数、`-v` 里 AF_UNIX 提示次数、容器内与宿主机计数差异 | **只报告**，不参与判定 |

这么分是有原因的：PRoot 拿不到邻居表是**当前测试环境的能力短板**，不是程序应有的性质。
若写成“邻居表必须为 0”，哪天 PRoot 补上了这个能力，测试反而会报错——那就成了惩罚变好。
所有参考值都是**运行时现取**的，不写死；设备网络状态变化（接口/路由增减）也不会造成假失败。
